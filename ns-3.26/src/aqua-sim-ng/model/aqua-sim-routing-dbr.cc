...
{
    std::cout<<"DBR_SendingTimer"<<std::endl;
	m_a->Send_Callback();
}
...
{
    int AquaNum=AquaSimAddress::ConvertFrom(GetNetDevice()->GetAddress()).GetAsNum();
    NS_LOG_DEBUG("at "<<Simulator::Now().GetSeconds()<<" Address"<<AquaNum<<
                 " recive packet"<<p->GetUid());
    NS_LOG_FUNCTION(this);

    DestTeamTag Tdstteam;
    p->PeekPacketTag(Tdstteam);
    uint16_t destteam=Tdstteam.GetDestTeam();
  ...
  if (model == 0)
	...
	{// although we eliminate the beacon pkt
	 ...
	 // furture control logic
       ...
       AquaNum<< ": beacon pkt is received.");
  ...
  p->RemovePacketTag(ptag);
	...
	{// Wow, it seems some one is broadcasting the pkt for us,
	 // so we need to dismiss the timer for the pkt
        ...
        NS_LOG_DEBUG("AquaSimDBR::Recv: address:" <<
        AquaNum << ": got the pkt I've sent");

        p=0;
		//drop(p, DROP_RTR_ROUTE_LOOP);
		return false;
	}
    NS_LOG_DEBUG("777"<<dbrh.GetMode());
    //std::cout<<"dst:"<<dst<<" addre: "<<GetNetDevice()->GetAddress()<<" team: "<<GetNetDevice()->GetSinkTeam()<<" destteam"<<destteam<<std::endl;
    if(GetNetDevice()->GetSinkTeam()==0)
    { if (dst == GetNetDevice()->GetAddress())
        {// packet is for me
        NS_LOG_DEBUG("AquaSimDBR::Recv: address:" <<
        AquaNum<< ": packet"<<p->GetUid()<<"is delivered!");
        ...
        {
        if(GetNetDevice()->GetSinkTeam()==destteam)
            {
            NS_LOG_DEBUG("AquaSimDBR::Recv: address:" <<
            AquaNum<< ": packet"<<p->GetUid()<<"is delivered!");
            if (!SendUp(p))
...
// The node will broadcast all RECOVERY packets, but
// it will drop the GREEDY packets from upper level.
void
AquaSimDBR::HandlePktForward(Ptr<Packet> p)
{
    NS_LOG_DEBUG("HandlePktForward");
  AquaSimHeader ash;
  DBRHeader dbrh;
//  Ipv4Header iph;
  AquaSimPtTag ptag;
  ...
  if (model == 0)
	...
	*/
    ...
    //@aloee
    //ash.SetSize(dbrh.Size() + IP_HDR_LEN);
    //NS_LOG_DEBUG("AquaSimDBR::SetSize" << dbrh.Size() + IP_HDR_LEN);
	ash.SetNextHop(AquaSimAddress::GetBroadcast());
    NS_LOG_DEBUG("1"<<dbrh.GetMode());
	switch (dbrh.GetMode())
 {
	case DBRH_DATA_GREEDY:
        NS_LOG_DEBUG("DBRH_DATA_GREEDY");
		//mn_->getLoc(&x, &y, &z);
      ...
      ", delta=" << delta);
		...
		break;
	case DBRH_DATA_RECOVER:
        NS_LOG_DEBUG("DBRH_DATA_RECOVER");
		if (dbrh.GetNHops() <= 0)
		{
         NS_LOG_DEBUG("AquaSimDBR::Recv: address:" <<
          GetNetDevice()->GetAddress() << ": drops pkt! (nhops < 0)");
      ...
      p=0;
		...
		break;
	default:
    NS_LOG_DEBUG("AquaSimDBR::Recv: Unknown data type!");
		return;
	}
...
AquaSimDBR::Recv(Ptr<Packet> p, const Address &dest, uint16_t protocolNumber)
{
  ...
  if (model == 0)
    ...
    NS_LOG_DEBUG("AquaSimDBR::SetSize" << ash.GetSize() + IP_HDR_LEN + 8);
    //iph.SetTtl(128);
		dbrh.SetMode(DBRH_DATA_GREEDY);
