...
void
AquaSimNetDevice::SetSinkTeam(uint16_t sinkTeam)
{
  NS_LOG_FUNCTION(this);
  m_sinkteam=sinkTeam;
}
...
AquaSimNetDevice::Send (Ptr< Packet > packet, const Address &dest, uint16_t protocolNumber)
{
  ...
  uint16_t curID =packet->GetUid();
  NS_LOG_DEBUG("ccccc"<<packet->GetSize());
  ash.SetSize(pktSize);
  ...
  if(m_routing)
    {//Note : https://www.nsnam.org/docs/release/3.24/doxygen/uan-mac-cw_8cc_source.html#l00123
      ...
      NS_LOG_DEBUG("Me(" << AquaSimAddress::ConvertFrom(GetAddress()).GetAsNum() << "): Sending packet to Routing layer : " << ash.GetSize() << " bytes ; " << ash.GetTxTime().GetSeconds() << " sec. ; Dest: " << ash.GetDAddr().GetAsInt() << " ; Src: " << ash.GetSAddr().GetAsInt() << " ; Next H.: " << ash.GetNextHop().GetAsInt());
      int _byte=ash.GetSize();
      double _time=Simulator::Now().GetSeconds();
      m_SentPktRecord[curID]=std::make_pair(_byte,_time);
      std::cout<<"m_SentPktRecord:"<< curID<<"  "<< ash.GetSAddr()<<" to "<<ash.GetDAddr()<<"  by"<< ash.GetNextHop().GetAsInt()<<"  "<<_byte<<"  "<<_time<<std::endl;
       Ptr<MobilityModel> model = GetNode()->GetObject<MobilityModel>();
      std::cout<<GetAddress()<<" ("<<model->GetPosition().x<<","<<model->GetPosition().y<<","<<model->GetPosition().z<<")"<<std::endl;
